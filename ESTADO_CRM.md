...
| **Plugin** | `vitacare-crm` **v1.15.4** |
| **DB schema** | **v5** (`vitacare_crm_db_version`) — sin cambios de esquema desde D-27; D-26 agregó `wp_vitacare_crm_email_campaigns` + `wp_vitacare_crm_campaign_recipients`; D-25 agregó `wp_vitacare_crm_link_clicks`; D-24 agregó `wp_vitacare_crm_leads` + columna `lead_id` en conversations; sin cambios de esquema en C-3/PR-6/PR-6b/D-19/D-20/C-6/D-22/D-23/D-31 |
| **Última actualización docs** | 2026-08-05 |
| **Último commit de referencia** | D-31 v1.15.4: diagnóstico no sensible de ingesta del webhook Messenger + reconocimiento de postback/handover/standby (este commit) |
...
| 2026-08-05 | **D-31 v1.15.4 — auditoría de ingesta de mensajes de Messenger**: con OAuth ya funcionando, Página conectada, webhook verificado/re-suscrito y el botón "Probar" de Meta confirmando éxito, la bandeja seguía vacía. Auditoría completa de `handle_post()` y `Vitacare_Crm_Channel_Messenger` no encontró ningún bug de persistencia demostrable por inspección estática (traversal de `entry[].messaging[]`, dedupe por `mid`, `object==='page'`, page ID, feature flag, todo correcto); sin acceso a los logs de producción no fue posible demostrar la causa raíz exacta del lado del servidor. Se entrega en su lugar la instrumentación no sensible pedida (Fase 1 del pedido): nueva clase `Vitacare_Crm_Webhook_Diagnostics` (opción `vitacare_crm_webhook_diag`, últimos 20 eventos, IDs enmascarados, nunca texto/tokens/App Secret/firma completa), registrada en cada retorno de `handle_post()` (channels_disabled/app_secret_missing/empty_body/signature_invalid/invalid_json/éxito/excepción) y con el detalle por evento de Messenger (tipo detectado, contacto/conversación/mensaje creados, motivo si se ignoró) — visible en `CRM VITACARE → Integraciones → Diagnóstico`, con botón "Limpiar registro". Se agregó también reconocimiento explícito (sin persistir, por diseño) de `postback`, `pass_thread_control`/`take_thread_control` y `standby` (Handover Protocol) — antes se descartaban sin ninguna etiqueta. El chequeo de Page ID ya existente ahora se etiqueta como `test_sample` cuando no coincide con la Página conectada, en vez de un genérico "skipped" indistinguible de un evento real. Nuevo test puro `tests/test-messenger-webhook-classification.php` (sin bootstrap WP) cubre la clasificación de message/echo/postback/delivery/read/handover/unknown — los casos que requieren persistencia real (contacto/conversación/mensaje creados exactamente una vez, dedupe, canal desactivado, página distinta) no se pudieron cubrir con test automatizado por falta de un entorno WP+MySQL en esta sesión; se verificaron por auditoría de código. Archivos: `class-vitacare-crm-webhook-diagnostics.php` (nuevo), `class-vitacare-crm-webhook.php` (instrumentación, sin cambiar el comportamiento de WhatsApp/Instagram), `class-vitacare-crm-channel-messenger.php` (clasificación + diagnóstico, sin cambiar qué se persiste), `class-vitacare-crm-integrations-page.php` (tabla + botón limpiar en pestaña Diagnóstico). No se tocó la app de Meta, suscripciones, tokens, permisos ni conversaciones existentes. **Pendiente real**: el usuario debe generar un evento nuevo (mensaje real o "Probar" de Meta) y revisar la tabla de diagnóstico para ver, con datos reales de producción, en qué punto exacto se está deteniendo — a partir de ahí se podrá aplicar una corrección dirigida en vez de seguir conjeturando | este commit |
