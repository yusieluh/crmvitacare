...
2. Leer este archivo.  
3. **No** rehacer PR-0…PR-5 ni C-1, C-2, C-4, C-5, C-7 (ya están en `main` v1.0.0).  
...
- Admin: Cuentas, WA Coexistence checklist, Facebook, Gmail, Credenciales
- DB upgrader a v2, logger en uploads protegido

Versión plugin: **1.0.0** en `vitacare-crm.php`.
...
| ID | Descripción |
|---|---|
| Ops | Desplegar ZIP en producción; configurar Meta + Google; cron Hostinger |
| **C-3** | Instagram messaging (oficial, vinculado a Page) |
| **PR-6** | Media WhatsApp (descargar, store opaco, deny HTTP, servir con cap) |
...
Tarea concreta: <describir C-3 / PR-6 / etc.>
```
